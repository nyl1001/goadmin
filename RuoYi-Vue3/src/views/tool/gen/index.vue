    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        ...
        <el-button
          ...
          plain
          icon="Download"
          @click="handleInitGenTable"
          v-hasPermi="['tool:gen:initTable']"
          :disabled="btnisShow"
        >初始化生成数据</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" width="330" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-tooltip content="同步" placement="top">
            <el-button link type="primary" icon="Refresh" @click="handleSynchDb(scope.row)" v-hasPermi="['tool:gen:sync']"></el-button>
          </el-tooltip>
...
import { listTable, initTable,previewTable, delTable, genCode, synchDb } from "@/api/tool/gen";
...
/** 初始化生成数据操作 */
function handleInitGenTable(row) {
...
function handlePreview(row) {
  previewTable(row.tableId).then(response => {
    preview.value.data = response.data.list;
