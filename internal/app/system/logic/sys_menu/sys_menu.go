...
func (s *sSysMenu) GetPermsUrlByUserId(ctx context.Context, userId int64) (perms map[string]bool) {
	...
	userPermKey := consts.CacheKeyPermsUrl + "." + strconv.FormatInt(userId, 10)
	has, err := cache.Exists(ctx, userPermKey)
	if err != nil {
		return
	}
	if has != 0 {
		iPerms, err := cache.Get(ctx, userPermKey)
		if err != nil {
	...
	//放入缓存
	cache.SetEX(ctx, userPermKey, perms, 60)
	return
}
